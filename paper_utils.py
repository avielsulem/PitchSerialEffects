...
def mgcv_fit(df_, model, columns, factors=False, newdata=[]):
    ...
    if type(factors) == list:
        for fac in factors:
            datar[columns.index(fac)] = base.as_factor(
                datar[columns.index(fac)])  # declaring y1 into a factor
...
def build_features(data_dict, alphas=[]):
    ...
    # whether subject responded correct or wrong
    corr = (Y == Ytrue).astype(int)
    ...
    trials = np.tile(np.linspace(
        0, d1.shape[1]-1, d1.shape[1]), (f1.shape[0], 1))
...
def load_data():
    ...
    fnames = ['complex_pure_mixed_no_missing',
              'complex_pure_mixed_missing_f', 'complex_pure_mixed_missing_f1']
    ...
    for i, fname in enumerate(fnames):
        ...
        data_dict, _ = filter_sub_trials(data,
                                         acc_lims=acc_lims,
                                         consistency_prm={
                                             'n_window': 10, 'w_window': 60, 'thresh': thresh},
                                         trial_lims=(0, 600))
